...
class _QEffAutoModelForImageTextToTextDualQPC:
    ...
    def kv_offload_generate(
        ...
        buffer_set = {
            "vision_embeds": np.zeros(vision_session.allowed_shapes[idx][1][1], dtype=np.float16),
        ...
        # print(f'Before pad  Vision embeds shape : {vision_outputs["vision_embeds"].shape}') #TODO": clean up
        # TODO : keep generic for QWEN - vision_session.allowed_shapes[idx][2][1][1] 
        vision_outputs["vision_embeds"] = np.pad(
            vision_outputs["vision_embeds"],
            pad_width=(
                (0, 0),
                (0, lang_session.allowed_shapes[0][1][1][1] - vision_session.allowed_shapes[idx][1][1][1]),
                (0, 0),
            ),  # pad axis=1 only
            mode="constant",
            constant_values=0,
        )
        # print(f'After pad Vision embeds shape : {vision_outputs["vision_embeds"].shape}') #TODO": clean up
